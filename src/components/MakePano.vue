      ...
      <div class="select-file">
        <template v-if="!imageUrl">
          ...
          <div class="upload-tips">
            <a class="download-test-file" @click="downloadTestImage" v-throttle
              >下载测试图片
            </a>
...
function makePano() {
  ...
  krpanoTool[makeFunc](inputFile.value)
    .then((reslut: IConvertPanoResult) => {
      packageVtour(reslut);
      hasError.value = false;
      loading.close();
    })
...
async function packageVtour(reslut: IConvertPanoResult) {
  ...
  const folder = zip.folder(
    "场景_" + reslut.dirName.replace(".tiles", "")
  ) as JSZip;
  ...
  await folder.loadAsync(
    urlToBinaryContent(
      new URL(`../assets/krpanoTemplate/files.zip`, import.meta.url).href
    )
  );
...
async function downloadTestImage() {
  ...
  await folder.loadAsync(
    urlToBinaryContent(
      new URL(`../assets/testImage/files.zip`, import.meta.url).href
    )
  );
